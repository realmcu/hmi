 ...
 * @return result
 */
/* check whether the KV name hits the reserved prefix (@see FDB_KVDB_CTRL_SET_RESERVED_PREFIX) */
static bool kv_name_hit_reserved(fdb_kvdb_t db, const char *key)
{
    size_t len;

    if (db->reserved_prefix == NULL || key == NULL) {
        return false;
    }
    len = strlen(db->reserved_prefix);

    return (len > 0) && (strncmp(key, db->reserved_prefix, len) == 0);
}

/*
 * Delete a KV. When allow_reserved is false, a key that hits the reserved prefix
 * (@see FDB_KVDB_CTRL_SET_RESERVED_PREFIX) is rejected with FDB_KV_NAME_ERR. The
 * Big File extension calls this with allow_reserved = true to manage its own
 * directory entries.
 */
fdb_err_t _fdb_kv_del_ex(fdb_kvdb_t db, const char *key, bool allow_reserved)
{
    fdb_err_t result = FDB_NO_ERR;
    ...
    if (!allow_reserved && kv_name_hit_reserved(db, key)) {
        FDB_INFO("Error: KV name (%s) uses the reserved prefix (%s).\n", key, db->reserved_prefix);
        return FDB_KV_NAME_ERR;
    }
...
{
    return _fdb_kv_del_ex(db, key, false);
}
 ...
 */
/*
 * Set a blob KV. When allow_reserved is false, a key that hits the reserved prefix
 * (@see FDB_KVDB_CTRL_SET_RESERVED_PREFIX) is rejected with FDB_KV_NAME_ERR. The
 * Big File extension calls this with allow_reserved = true to write its own
 * directory entries.
 */
fdb_err_t _fdb_kv_set_blob_ex(fdb_kvdb_t db, const char *key, fdb_blob_t blob, bool allow_reserved)
{
    fdb_err_t result = FDB_NO_ERR;
...
{
    return _fdb_kv_set_blob_ex(db, key, blob, false);
}
...
#pragma GCC diagnostic pop
    ...
    case FDB_KVDB_CTRL_SET_NOT_FORMAT:
        ...
        break;
    case FDB_KVDB_CTRL_SET_RESERVED_PREFIX:
        /* this change MUST after database initialization */
        db->reserved_prefix = (const char *) arg;
        break;
    }
}
