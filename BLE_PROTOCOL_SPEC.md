...
v-length 仍然占用传输字节流中的 **1 个字节**，不可省略。
...
- 在 L0 之上实现可靠传输（发送 + 接收）
- L0 MTU = 20 bytes，L1 实现 **504 bytes MTU**（内部拆包/组包）
...
#### L2 数据包结构（2~504 bytes）

```
┌───────────────────┬───────────────────────┐
│  L2 Header (2B)   │  L2 Payload (0~502B)  │
└───────────────────┴───────────────────────┘
```
...
|------|------|------|
| Reserve | 7 bits | 保留 |
| Key Value Length (v-length) | 9 bits | 该 key 的 value 字节数 |
...
手机                                    设备
 ...
 │◀─── XFER_DATA_ACK  (0x04) [seq=0] ───│  │ 逐块传输，每块须等 ACK 后再发下一块
 │──── XFER_DATA      (0x03) [seq=1] ───▶│  │ 若 ACK.status ≠ 0x00，重传同一块
 │◀─── XFER_DATA_ACK  (0x04) [seq=1] ───│  ┘ 连续失败 3 次后发送 XFER_ABORT
 │              …                         │
...
| chunk_size | 2 bytes | 请求的单块字节数，有效范围 1~496（Big-Endian）|
...
> 手机在收到对应 seq 的 XFER_DATA_ACK（status=`0x00`）之前，不得发送下一块。

---

#### 0x04 — 数据块确认（XFER_DATA_ACK）

**方向**：设备 → 手机 | **Value（3 bytes）**：

| 字段 | 大小 | 说明 |
|------|------|------|
| seq | 2 bytes | 被确认的块序号（Big-Endian）|
| status | 1 byte | `0x00`=成功；`0x01`=序号错误；`0x02`=写入失败 |

> status ≠ `0x00` 时，手机应重传相同 seq 的块；连续失败 3 次后发送 XFER_ABORT（reason=`0x01`）。
