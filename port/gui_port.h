/*
 * Copyright (c) 2026 Realtek Semiconductor Corp.
 * All rights reserved.
 *
 * Licensed under the Realtek License, Version 1.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License from Realtek
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

/**
 * @file gui_port.h
 * @brief HoneyGUI port layer interface for Ameba platform
 */

#ifndef __GUI_PORT_H__
#define __GUI_PORT_H__

#include <stdint.h>
#include <stdbool.h>

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Initialize acceleration engine
 */
void gui_port_acc_init(void);

/**
 * @brief Initialize display controller
 */
void gui_port_dc_init(void);

/**
 * @brief Initialize OS abstraction layer
 */
void gui_port_os_init(void);

/**
 * @brief Initialize input device
 */
void gui_port_indev_init(void);

/**
 * @brief Initialize filesystem
 */
void gui_port_fs_init(void);

/**
 * @brief Initialize flash translation layer
 */
void gui_port_ftl_init(void);

#ifdef __cplusplus
}
#endif

#endif /* __GUI_PORT_H__ */
