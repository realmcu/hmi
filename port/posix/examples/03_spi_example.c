...
void example_spi(void)
{
    ...
    posix_ioctl(spi, POSIX_SPI_IOCTL_CS_TAKE);   /* CS 拉低 */
    ...
    posix_ioctl(spi, POSIX_SPI_IOCTL_CS_RELEASE); /* CS 拉高 */

    /* uint16_t flash_id = (rx_buf[3] << 8) | rx_buf[4]; */

    /* === 4. 只发不收（半双工） === */
    uint8_t cmd[] = { 0x06 };   /* W25Q Write Enable */
    posix_ioctl(spi, POSIX_SPI_IOCTL_CS_TAKE);
    posix_write(spi, cmd, sizeof(cmd));
    posix_ioctl(spi, POSIX_SPI_IOCTL_CS_RELEASE);
