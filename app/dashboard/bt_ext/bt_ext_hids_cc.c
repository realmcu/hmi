 ...
 *
 * NOTE: profile only. Sending media-key reports (next/prev/play-pause/volume)
...
/* Consumer Control report map — identical to dashboard_hids_cc.c */
...
/* Service id assigned by server_add_service(); needed to send notifications. */
...
/* Map an attribute index to a human-readable name, for logging any HID operation */
		...
		uint16_t attrib_index, uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
	...
	(void)service_id;
	/* bee stack slices long reads from the full value */
