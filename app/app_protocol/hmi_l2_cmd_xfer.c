...
#include "hmi_l2.h"
#include "hmi_proto.h"
#include "hmi_protocal_task.h"    /* hmi_proto_post_call + l2_msg_t */
#include "proto_log.h"
#include "app_event.h"
 ...
 * on_cmd_xfer) so an EVT_BLE_DISCONNECTED subscriber can abort a half-written
 * file — the actual reset is bounced onto l2_task to serialise fdb_bf_abort
 * against any DATA-frame fdb_bf_append still queued ahead of it. */
...
static void hmi_l2_xfer_reset(void)
{
...
/* Runs on l2_task (posted by the disconnect subscriber). Executing on l2_task
 * is essential: it serialises fdb_bf_abort against any DATA-frame fdb_bf_append
 * still queued ahead of us. Reset on app_task would race the flash writer. */
static void xfer_reset_on_l2_task(l2_msg_t *p_msg)
{
    (void)p_msg;
    hmi_l2_xfer_reset();
}

/* Runs on app_task (event-bus dispatcher). Bounces the actual reset onto
 * l2_task so it doesn't race with in-flight DATA frames. */
static void on_ble_disconnected(app_event_id_t id, const void *payload,
                                size_t len, void *user)
{
    (void)id; (void)payload; (void)len; (void)user;
    (void)hmi_proto_post_call(xfer_reset_on_l2_task, NULL);
}
