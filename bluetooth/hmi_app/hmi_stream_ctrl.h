 ...
 *        init (example_gui_stream.c, via stp_instance_create()); this just
 *        forwards that single shared instance (same handle as
 *        app_stream_transport_get()) -- borrowed, never created here.  Returns
 *        NULL until the app has created it.  Kept (hmi_l2_stream_*) for
 *        backward compatibility; new code may call app_stream_transport_get().
 */
