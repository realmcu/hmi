...
#define FDB_KVDB_CTRL_SET_RESERVED_PREFIX 0x0C          /**< set the reserved KV name prefix; KVs using this prefix are rejected by the public set/del API (used by the Big File extension), this change MUST after database initialization */
...
typedef enum {
    ...
    FDB_SAVED_FULL,
    FDB_INIT_FAILED,
    FDB_NOT_FOUND,                                /**< the target (KV / Big File / address) is not found */
    FDB_NO_SPACE,                                 /**< no enough space for the operation */
    FDB_UNSUPPORTED,                              /**< the operation is not supported on current storage medium */
    FDB_BUSY,                                     /**< no free resource (e.g. Big File open handle) */
    FDB_INVALID_PARAM,                            /**< invalid parameter */
} fdb_err_t;
...
struct fdb_kvdb {
    ...
    const char *reserved_prefix;                 /**< reserved KV name prefix set by FDB_KVDB_CTRL_SET_RESERVED_PREFIX; NULL means no reservation */
